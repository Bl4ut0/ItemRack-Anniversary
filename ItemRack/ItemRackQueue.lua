...
function ItemRack.ResolveProxy(itemID)
...
function ItemRack.GetProxyCooldown(itemID)
	if not GetItemCooldown then return nil end
	local proxy = ItemRack.ResolveProxy(itemID)
...
function ItemRack.ApplyProxyCooldown(itemID, start, duration, enable)
	local proxyStart, proxyDuration, proxyEnable = ItemRack.GetProxyCooldown(itemID)
	if proxyStart then
...
function ItemRack.GetProxyCooldownLeft(itemID)
	if not GetItemCooldown then return nil end
	local proxy = ItemRack.ResolveProxy(itemID)
	if not proxy then return nil end
...
function ItemRack.GetItemCooldownLeft(itemID)
	...
	local proxied = ItemRack.GetProxyCooldownLeft and ItemRack.GetProxyCooldownLeft(numericID)
